...
function* getJobsPosted(action) {
  ...
  try {
    ...
    if (res) {
      ...
      yield put({ type: SAVE_POSTED_JOB_ERR, payload: res.message });
    }
  } catch (err) {
    yield put({ type: SAVE_POSTED_JOB, payload: null });
    yield put({ type: SAVE_POSTED_JOB_ERR, payload: err.message });
